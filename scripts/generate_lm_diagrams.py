...
def make_fermat_snell_huygens_animation(path: Path) -> None:
    ...
    source = np.array([-1.85, 1.85])
    target = np.array([1.78, -1.78])
    crossing_grid = np.linspace(-1.65, 1.65, 500)
    times = n1 * np.sqrt((crossing_grid - source[0]) ** 2 + source[1] ** 2)
    times += n2 * np.sqrt((target[0] - crossing_grid) ** 2 + target[1] ** 2)
    x_star = crossing_grid[int(np.argmin(times))]
    alternatives = [-1.15, -0.35, 0.85, 1.35]

    fig, (ax_wave, ax_path) = plt.subplots(1, 2, figsize=(12.8, 5.9), gridspec_kw={"wspace": 0.22})
    fig.patch.set_facecolor("#F7F3EC")
    ...
    def draw_frame(frame: int):
        ...
        for ax in (ax_wave, ax_path):
            style_axes(ax)
            ax.axhspan(0, 2.55, color="#FFFDF8", zorder=0)
            ax.axhspan(-2.55, 0, color="#E8F2F7", zorder=0)
            ax.axhline(0, color="#7E7468", linewidth=1.6, zorder=2)
            ax.text(-2.62, 0.12, "fast medium", fontsize=9.8, color="#4B463F")
            ax.text(-2.62, -0.32, "slow medium", fontsize=9.8, color="#4B463F")
            ax.set_xticks([])
            ax.set_yticks([])
        ...
        ax_path.set_title("Fermat/Snell: the same bend as least time", fontsize=12.4, color="#2F2F2F")
        ax_path.set_xlim(-2.75, 2.75)
        ax_path.set_ylim(-2.35, 2.35)
        ax_path.set_aspect("equal", adjustable="box")
        ax_path.scatter([source[0], target[0]], [source[1], target[1]], s=72, color="#2F2F2F", edgecolor="white", linewidth=1.0, zorder=6)
        ax_path.text(source[0] - 0.16, source[1] + 0.18, "source", fontsize=9.5, color="#4B463F")
        ax_path.text(target[0] - 0.18, target[1] - 0.32, "detector", fontsize=9.5, color="#4B463F")
        ax_path.axvline(x_star, color="#D7C6AA", linewidth=1.0, linestyle="--", alpha=0.7)

        for xc in alternatives:
            ax_path.plot([source[0], xc, target[0]], [source[1], 0, target[1]], color="#BDB4A7", linewidth=1.3, alpha=0.5)

        highlight_alpha = min(max((progress - 0.28) / 0.35, 0.0), 1.0)
        ax_path.plot(
            [source[0], x_star, target[0]],
            [source[1], 0, target[1]],
            color="#2F2F2F",
            linewidth=3.0,
            alpha=highlight_alpha,
        )
        ax_path.scatter([x_star], [0], s=62, color="#2F2F2F", edgecolor="white", linewidth=1.0, alpha=highlight_alpha, zorder=7)
        ax_path.text(
            0.05,
            0.91,
        ...
        ax_path.text(
            0.95,
            0.07,
            "Fermat names the global path;\nHuygens supplies the local mechanism.",
            transform=ax_path.transAxes,
            ha="right",
            va="bottom",
            fontsize=9.5,
