...
def background():
    ...
    text(d,(38,23),"Whole turns · the phase left over",31,bold=True)
    ...
    for pane,box in enumerate(PANELS):
        ...
        subtitle="Longer wavelength · λ = 1" if pane==0 else "Shorter wavelength · λ = 1/9"
    ...
    text(d,(39,818),"Full phase:  φ = 2πn + θ",23,INK)
...
def checks():
    ...
    assert np.allclose(PHASES[:,1],9*PHASES[:,0],rtol=0,atol=3e-14)
    assert np.allclose(np.exp(1j*PHASES),np.exp(1j*REMAINDERS),atol=2e-14)
    assert np.allclose(np.degrees(REMAINDERS),[[45,45],[59.4,174.6],[73.8,304.2]],atol=2e-11)
    max_step=0.
