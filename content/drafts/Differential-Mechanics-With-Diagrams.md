...
Phase space, or position/momentum space, is the arena in which Hamiltonian Mechanics takes place. To appreciate the Hamiltonian formulation, we need to understand the origin, geometry, and role of this space.

### From Histories to Instantaneous States

The central objects in Hamiltonian mechanics, the state variables and the Hamiltonian function on those variables, are obtained from Lagrangian mechanics. We know that Lagrangian mechanics selects physical paths from path space by extremizing action. The task we must now perform is to extract the essential objects of a local theory of instantaneous state from the global Lagrangian theory.

As we know from Lagrangian mechanics, we map spacetime variables to physical histories through their contribution to the variation of the action. By integrating the action integral by parts, we can separate those contributions into the variation of the interior of the path and the variation of the endpoints. The former yields the Euler-Lagrange equations of motion. That is, the equations of motion must be consistent with the path taken once we know the endpoints. But what is the significance of the contribution to the action from displacing the endpoints? It does not answer, “How does the path taken from A to B change the action?” but rather, “How does an infinitesimal change in position, while holding time constant, change the action?” If we imagine chopping a path into smaller and smaller pieces, in a sense all that is left is a series of boundary terms, each corresponding to a successive instantaneous state. The boundary term records how the action changes when that state is infinitesimally displaced, and the coefficient of the displacement, or we might say the measure of the displacement’s effect on the action, will become the momentum.
...
As any two numbers can be combined via addition or multiplication to produce a new number, so any two functions on phase space can be combined to produce a new function. When the rule of combination reflects the flow/evolution relationship between functions, we get the algebra of Hamiltonian mechanics, or Poisson algebra. In that algebra, Hamilton's equations, conservation laws, and other structural relationships become visible as algebraic identities.

### What We Mean by an Algebra

Before exploring Poisson algebra specifically, it is worth reminding ourselves what the word algebra means. An algebra begins with some class of objects and then specifies what operations may be performed on those objects and what properties those operations satisfy. Numbers, with operations of addition and multiplication satisfying the properties of associativity and commutativity, are the familiar example. Different algebras may involve different objects, operations, and properties, but these differences fall into a common taxonomy.
...
| Internal | Takes allowed objects as input and returns another allowed object of the same kind | Sum of two numbers is a number |
...
| Category | Meaning | Example identity |
|---|---|---|
| Symmetry property | Governs what happens when inputs are swapped | $2+3=3+2$ |
| Grouping property | Governs what happens when an operation is repeated | $(2+3)+4=2+(3+4)$ |
| Linearity property | Governs behavior under sums and scalar multiples | $5(2+3)=5\cdot2+5\cdot3$ |
| Compatibility property | Governs how two different operations interact | $2\cdot(3+4)=2\cdot3+2\cdot4$ |

### Poisson Algebra as a Lie Algebra

With this framing in mind, we can now turn to Poisson algebra. Before discussing the specific operations and properties that form the Poisson algebra notation, let's first look at its geometric meaning. A Poisson algebra belongs to the broader class of Lie algebras, in which we represent a finite transformation on a manifold by an infinitesimal displacement tangent to the transformation.

This picture is captured schematically as a differential equation that represents the action of an infinitesimal generator on a finite transformation. The generator acts on the current state to produce its next infinitesimal change.
...
With $g$ held fixed, $\{f,g\}$ is the change of $f$ when the state is moved along the vector field determined by $g$:

```math
\{f,g\}=\frac{df}{ds},
\quad\text{where }s\text{ is the parameter along the flow generated by }g.
```

The result is another smooth function on phase space. It goes to zero when $f$ does not change along that flow.
...
Rather than saying that different nestings are equal, it says that the cyclic sum closes to zero.
