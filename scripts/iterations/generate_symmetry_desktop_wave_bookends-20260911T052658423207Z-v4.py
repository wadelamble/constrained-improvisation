...
VERSION_NUMBER = 4
VERSION = f"{CANONICAL}-v{VERSION_NUMBER}-soft-wave-bookends"
...
POSITIVE = np.asarray((166.0, 109.0, 117.0))
NEGATIVE = np.asarray((88.0, 121.0, 158.0))
NEUTRAL = (POSITIVE + NEGATIVE) / 2
COLOR_SCALE = 1.3
PAPER = (247, 242, 232)
SCREEN_INK = (57, 57, 63)
COLOR_DESCRIPTION = "continuous muted blue-purple-red: mix(blue, red, (1+tanh(Re(E)/1.3))/2); purple is zero; no contour strokes"
...
def wave_frame(field, index, opening):
    ...
    # A single smooth signed transfer function, with approximately constant
    # lightness: negative blue -> zero purple -> positive dusty red. Purple
    # denotes zero signed field, not a third physical wave. The previous
    # black-at-zero map and sublinear power produced apparent dark outlines;
    # neither is used here. No blur or edits to the complex field are needed.
    mix = (1 + np.tanh(signed / COLOR_SCALE)) / 2
    rgb = NEGATIVE + (POSITIVE - NEGATIVE) * mix[..., None]
    ...
    for y0, y1 in ((0, aperture_top), (aperture_bottom, HEIGHT)):
        draw.rectangle((wall_x - 7, y0, wall_x + 7, y1), fill=SCREEN_INK + (255,))
        draw.line((wall_x - 8, y0, wall_x - 8, y1), fill=PAPER + (145,), width=1)
    # Caps at the two edges make the single finite opening explicit. The
    # offset bracket is an annotation, not another physical barrier. Its
    # exact endpoints use the same aperture bounds as the propagation model.
    for edge in (aperture_top, aperture_bottom):
        draw.line((wall_x - 8, edge, wall_x + 8, edge), fill=PAPER + (230,), width=2)
    bracket_x = wall_x + 25
    draw.line((bracket_x, aperture_top, bracket_x, aperture_bottom), fill=PAPER + (125,), width=1)
    for edge in (aperture_top, aperture_bottom):
        draw.line((bracket_x - 5, edge, bracket_x + 5, edge), fill=PAPER + (190,), width=1)
    draw.text((wall_x + 20, 24), "screen", font=SMALL_FONT, fill=PAPER + (255,))
    # A compact paper label keeps the opening legible over either wave color.
    box = draw.textbbox((0, 0), "opening", font=SMALL_FONT)
    label_x, label_y = bracket_x + 11, aperture_top + 14
    draw.rounded_rectangle((label_x - 6, label_y - 2, label_x + box[2] + 6, label_y + 24), radius=3, fill=PAPER + (215,))
    draw.text((label_x, label_y), "opening", font=SMALL_FONT, fill=SCREEN_INK + (255,))
    detector_x, _ = xy(physics.Z2, 0)
    draw.line((detector_x, 0, detector_x, HEIGHT), fill=SCREEN_INK + (235,), width=3)
    draw.text((detector_x + 15, 24), "detector", font=SMALL_FONT, fill=PAPER + (255,))
    ...
    for label, point in (("A", xy(-physics.Z1, 0)), ("B", xy(physics.Z2, b))):
        ...
        draw.text((x + 13, y - 15), label, font=LABEL_FONT, fill=PAPER + (255,))
    return image
...
def save_wave_previews(field, prefix):
    ...
    for row, opening in enumerate((True, False)):
        for col, index in enumerate((0, 24, 71)):
            ...
            draw.text((col * 640 + 14, row * 386 + 365), f"{'opening' if opening else 'closing'}: frame {index}", fill=desktop.old.INK)
