...
def make_huygens_snell_synthesis(path: Path) -> None:
    ...
    def draw_frame(frame: int):
        ...
        # Blue Huygens reference above: the same construction in the fast medium.
        old_fast_phase = phase - fast_step
        for y_target in [0.46, 0.78, 1.10, 1.42]:
            u = (y_target - old_fast_phase * d1[1]) / w1[1]
            center = old_fast_phase * d1 + u * w1
            if center[1] > 0.16 and -2.5 < center[0] < 2.5:
                draw_clipped_circle(ax, center, fast_step, True, BLUE, 1.25, 0.38, 12)
                ax.scatter([center[0]], [center[1]], s=18, color=BLUE, edgecolor="white", linewidth=0.5, zorder=16)
...
if __name__ == "__main__":
    make_huygens_snell_synthesis(OUT / "lm-huygens-snell-synthesis-review.mp4")
