...
The three-pane body is recalculated from the same 49-slit mask, then stream-
copied between the wave segments. Every decoded body frame is checked against
that new render. The old continuous-opening body cannot be reused silently.
"""
...
VERSION_NUMBER = 5
VERSION = f"{CANONICAL}-v{VERSION_NUMBER}-49-physical-slits"
...
def wave_frame(field, index, opening):
    ...
    for lower, upper in physics.opaque_intervals(Y_MIN, Y_MAX):
        _, y0 = xy(0, upper)
        _, y1 = xy(0, lower)
    ...
    draw.text((wall_x + 20, 47), "49 slits", font=SMALL_FONT, fill=PAPER + (255,))
