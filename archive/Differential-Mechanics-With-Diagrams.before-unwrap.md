...
question of why dynamical laws exist. And indeed, the central objects of the differential formulation are derived from the global formulation. "Hamiltonian Mechanics" is the name we give to the modern version of the local, differential perspective. Instead of selecting a whole history at once, it treats motion as a law on instantaneous state. It says "give me the state and the symmetries, and I'll give you the next state." In that sense it is the fully differential and kinematical formulation of mechanics. 
...
structure is in hand, it reveals that the time evolution of conservative systems preserves the information embodied in ensembles of states. That same kinematic structure can then be repackaged as a generator algebra, which later provides a bridge to quantum mechanics.
