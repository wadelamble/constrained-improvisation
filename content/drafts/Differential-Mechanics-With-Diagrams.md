...
Given two functions $f$ and $g$ on phase space, we can ask how $f$ changes along the flow generated by $g$, or, just as well, how $g$ changes along the flow generated by $f$. As any two numbers can be combined via addition or multiplication to produce a new number, so any two functions on phase space can be combined to produce a new function when the rule for combining them reflects this flow/evolution relationship. We call such a system of rules for combining objects an algebra. In the algebra of Hamiltonian mechanics, called Poisson algebra, Hamilton's equations, conservation laws, and other structural relationships become visible as algebraic identities.

[cut]
### Canonical Transformatin Generators vs symmetry generators (to be moved below)
We have seen the reciprocal pairing of position and momentum in Hamiltonian mechanics in the symplectic form, where changing one side of the area element requires a compensating change in the other to preserve area, and in Hamilton's equations of motion, where variation of $H$ with respect to position determines the rate of change of momentum, and vice versa. In the algebraic formulation, the notation says explicitly that $p$ generates $q$, and $q$ generates $p$.

The $p \to q$ direction feels familiar. Noether’s theorem has made us comfortable with the idea that where there is a symmetry, such as picking the experiment up and moving it over there, or repeating it tomorrow, there is an associated conserved quantity paired with that symmetry. But what are we to make of the $q \to p$ direction? What can it mean for position to generate momentum? Is position conserved? Is momentum a symmetry? Surely not.
...
But this relationship does not mean that momentum is a symmetry. A physical symmetry is a transformation that preserves the action. Its infinitesimal generator is the direction tangent to that transformation. A canonical transformation preserves the symplectic form, and a function on phase space generates such a transformation by determining a Hamiltonian vector field. 

While the Hamiltonian reciprocal generator relation does live inside a genuine symmetry structure, position and wave number \(which we can associate with momentum\) are symmetry operations on the space of waves. Fourier transform exchanges the two.Shifting in $x$ corresponds to multiplying by a $k$-phase, while multiplying by an $x$-phase shifts $k$. In that setting, $k$ generates translations in $x$, and $x$ generates translations in $k$. Quantum mechanics will turn this reciprocal Fourier symmetry into the canonical commutation relation between position and momentum.
[endcut]

### What we mean by an algebra

Before exploring Poisson algebra specifically, let's remind ourselves what an "algebra" is. It begins with some class of objects and then specifies what operations may be performed on those objects and what laws those operations satisfy. Numbers with addition and multiplication and associativity and commutativity are the familiar example. Different algebras may involve different objects, operations, and laws, but these differences fall into a common taxonomy. 
...
With this framing in mind, we can now turn to Poisson algebra. Before discussing the specific operations and rules that form the Poisson algebra notation, let's first look at its geometric interpretation. A Poisson algebra belongs to the broader class of Lie algebras, in which we represent a finite transformation on a manifold by an infinitesimal displacement tangent to the transformation. This picture leads to a differential equation with a schematic form:
...
The matrix form appears when the transformations are linear transformations of a vector space, such as rotations, boosts, or internal symmetry transformations. In that setting, both the finite transformations and their infinitesimal generators can be represented by matrices.

For a flow on phase space, the finite transformation is not usually represented as a matrix. It is a map from phase space to itself. It is therefore easier to write the same idea in terms of a moving point

The matrix form appears when a transformation can be represented by one global, linear rule acting on a vector space, such as a rotation, boost, or internal symmetry transformation. The same matrix acts everywhere, taking each vector to its transformed vector.

For a flow on phase space, the finite transformation is usually better understood as a map from phase space to itself generated by local instructions. The vector field assigns a direction to each point of phase space, and different points may be assigned different directions and speeds. It is therefore easier to write the same idea in terms of a moving point
...
We can read these equations in shorthand as "momentum generates position translations" and "position generates momentum translations." At this point the attentive reader may scratch their head and say "wait, I expect from Relativity and Noether's theorem that momentum generates position, but what could it possibly mean for position to generate momentum. Conserved position? Momentum symmetry? This makes no sense. The key to resolving this confusion is that momentum and position are bidirectionally paired in the case of generating canonical transformation, not in the case of generating symmetry transformations. Momentum is not a symmetry of the physics. However, there is a physical way to understand the pairing in phase space. 

Imagine a downhill skier. The mountain's incline determines how their momentum changes, and that incline is a function of position. So $\dot p$ is determined by position. This example is illustrative because it realizes the idea of a potential gradient as an actual hill. Whatever the details of the system, if we choose $p$ and $q$ so that $H = K(p) + V(q)$, then the more $H$ changes with respect to $q$, the greater the rate of change of momentum. This is what is meant, dynamically, by saying that $q$ generates $p$.

While position/momentum pairing does not belong to the symmetries of classical physics, it does appear in the symmetry space of wavefunctions \(if we associate wave number with momentum.\) Fourier transform exchanges the two. Shifting in $x$ corresponds to multiplying by a $k$-phase, while multiplying by an $x$-phase shifts $k$. In that setting, $k$ generates translations in $x$, and $x$ generates translations in $k$. Quantum mechanics will turn this reciprocal Fourier symmetry into the canonical commutation relation between position and momentum.

The connection with the symplectic structure is not accidental. In classical phase space, a position displacement and a momentum displacement can be represented by vectors such as $(a,0)$ and $(0,b)$. The symplectic form measures their oriented phase-space area:
...
while a wave-number, or momentum, shift acts by phase modulation:
