...
[ANIMATION]
Opening frame: The six exact deck states appear as labeled nodes, with
three of them carrying nonzero probability weights.
Motion: Apply the same permutation to every node at once, and move the
probability weights with the nodes rather than changing their values.
Closing frame: The same three weights appear in new locations on the
same six-state space.
Purpose: Establish the discrete prototype of measure-preserving
evolution before passing to continuous phase space.
...
[ANIMATION]
Opening frame: A cloud of initial conditions is shown in
position-velocity space around a mean starting state.
Motion: Each point in the cloud evolves under the same conservative
equation of motion, so the cloud deforms and moves without collapsing
into a sharper or flatter distribution.
Closing frame: A later cloud appears with a different shape and
location, but as a transported image of the earlier one.
Purpose: Move from the discrete permutation picture to a continuous
ensemble whose information is carried rather than erased.
...
[ANIMATION]
Opening frame: A contour map of the stream function $\psi(x,y)$ is shown
with a small material patch of fluid marked out.
Motion: Velocity arrows appear tangent to the contours; the patch is
carried by the flow, shearing and stretching as contour density changes
the local speed.
Closing frame: The patch is visibly distorted but has kept the same
area.
Purpose: Establish the incompressible-fluid prototype of crossed motion
and preserved area before the Hamiltonian version.
...
[ANIMATION]
Opening frame: A single path from $t_1$ to $t_2$ is drawn.
Motion: First the interior of the path is perturbed while the endpoints
stay fixed, then the interior is held steady while one endpoint is
nudged.
Closing frame: The two kinds of change are labeled separately as bulk
variation and boundary variation.
Purpose: Show how a global variational principle begins to split into an
equations-of-motion piece and a state-displacement piece.
...
[DIAGRAM]
Show: A worldline endpoint with an infinitesimal spacetime displacement
$\delta x^\mu$, annotated first by the covector pairing
$p_\mu\,\delta x^\mu$ and then by its split into
$p_i\,\delta q^i - E\,\delta t$.
Purpose: Connect the Hamiltonian boundary pairing to the more familiar
relativistic energy-momentum structure.
...
[ANIMATION]
Opening frame: A single point in phase space is shown with one tiny
displacement, representing the one-state question encoded by
$p_i\,\delta q^i$.
Motion: Nearby states are introduced until the picture becomes a tiny
patch spanned by the paired directions $dq^i$ and $dp_i$.
Closing frame: The oriented area element $dq^i \wedge dp_i$ is attached
to the patch, ready to be carried by a flow.
Purpose: Mark the change of question from one displaced state to
ensembles of nearby states.
...
[DIAGRAM]
Show: A spacetime diagram with a worldline beside a phase-space plot
containing only a point and axes.
Purpose: Clarify the shift from a geometry in which motion is already
drawn to one in which motion must be generated by a law.
...
[ANIMATION]
Opening frame: A contour map of $F(q,p)$ is shown on the phase-space
plane with no arrows attached.
Motion: Local gradient arrows appear briefly and are then rotated into a
tangent flow field; arrow lengths grow where the contours are more
tightly packed.
Closing frame: The full $q,p$ plane carries a vector field tangent to
the level sets of $F$.
Purpose: Show how a phase-space function becomes a local rule of
motion.
...
[ANIMATION]
Opening frame: A small phase-space patch is marked out inside the vector
field generated by $F$.
Motion: The patch is carried by the flow, changing shape through shear,
stretch, and rotation while an area indicator remains fixed.
Closing frame: The patch is visibly distorted but unchanged in area.
Purpose: Make symplectic area preservation concrete before the
...
[DIAGRAM]
Show: A short spacetime displacement decomposed into temporal and
spatial pieces, with the action contribution labeled as
$-\int E\,dt + \int p_x\,dx + \int \cdots$.
Purpose: Make the phrase "energy generates time" visible as a
...
[DIAGRAM]
Show: A variable trade from $(q,\dot q)$ to $(q,p)$, with the momentum
relation $p_i=\partial L/\partial \dot q^i$ as the bridge and
$H(q,p)=p_i\dot q^i-L$ as the new phase-space function.
Purpose: Present the Legendre transform first as a change of
description, before the later differential comparison.
...
[DIAGRAM]
Show: Four stacked layers labeled function, vector field, flow, and
canonical transformation, with the note that a flow is one-parameter and
each fixed parameter value gives one canonical transformation.
Purpose: Keep the objects of the Lie-style discussion from blurring
together.
...
[DIAGRAM]
Show: A compact three-line panel linking Hamilton's equations,
$\dot f=\{f,H\}$, and the vanishing-bracket statements for conservation
and symmetry.
Purpose: Display the algebraic compression achieved by the Poisson
bracket.
...
[DIAGRAM]
Show: A left-to-right map from classical to quantum:
functions -> operators,
Poisson brackets -> commutators,
phase-space state geometry -> quantum state geometry.
Purpose: End by making explicit what transports forward and what is
replaced.
