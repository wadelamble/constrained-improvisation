...
[cut]With this framing in mind, we can now turn to Poisson algebra. Before discussing the specific operations and rules that form the Poisson algebra notation, let's look first at its geometric interpretation. A Poisson algebra belongs to the broader class of Lie algebras.  In the usual symmetry setting, the finite objects are transformations that preserve some invariant quantity. 
[endcut]
[likelycut]
For example, planar rotations preserve the Euclidean length $x^2+y^2$, and may be written as a one-parameter family of matrices $R_{\theta}$. The infinitesimal objects are the generator matrices $A$ from which the finite transformations are built. In that setting one writes
[endlikelycut]

With this framing in mind, we can now turn to Poisson algebra. Before discussing the specific operations and rules that form the Poisson algebra notation, let's first look at its geometric interpretation. 

[possiblecut]
A Poisson algebra belongs to the broader class of Lie algebras, in which we represent a transformation on a manifold with an object in a tangent space to the manifold at the identity element that acts on the state to generate the next infinitesimal step of the transformation. 
[endpossiblecut]

A Poisson algebra belongs to the broader class of Lie algebras, in which we represent a finite transformation on a manifold by an infinitesimal displacement tangent to the transformation. This picture leads to a differential equation with a schematic form:
...
so exponentiation turns an infinitesimal generator into a parameterized family of finite transformations. The ability to map the infinitesimal displacement to the finite transformation depends on the existence of a the symmetry, or some similar rule-boundedness, that allows repeated action of the tangent to produce the transformation.
[endcut]
[cut]
