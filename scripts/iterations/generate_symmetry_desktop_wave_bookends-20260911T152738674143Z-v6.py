...
The detector carries its actual intensity in a brightness strip and the same
linear-scale profile used by the three-pane diagram. The validated 49-slit
body follows the four-second wave intro, with no closing wave segment.
Every decoded body frame is checked against the matching input render.
"""
...
VERSION_NUMBER = 6
VERSION = f"{CANONICAL}-v{VERSION_NUMBER}-wave-and-intensity-intro"
...
POSITIVE = np.asarray((181., 78., 67.))
NEGATIVE = np.asarray((43., 91., 143.))
NEUTRAL = np.asarray(desktop.old.PANEL, dtype=float)
COLOR_SCALE = 1.4
PAPER = tuple(desktop.old.BG)
SCREEN_INK = tuple(desktop.old.INK)
COLOR_DESCRIPTION = "blue / warm-paper / red signed field; opacity .84*abs(tanh(Re(E)/1.4)); one global transfer, no black contours"
# One integrated scene: source, perforated barrier, wave, projection strip,
# and its intensity profile share a vertical coordinate. Keep circular
# wavefronts circular: the wave crop has exactly equal x/y physical scales.
WAVE_LEFT, WAVE_TOP, WAVE_RIGHT, WAVE_BOTTOM = 52, 128, 932, 638
WAVE_WIDTH, WAVE_HEIGHT = WAVE_RIGHT - WAVE_LEFT, WAVE_BOTTOM - WAVE_TOP
VIEW_Y_MIN, VIEW_Y_MAX = -3.45, 3.45
PIXELS_PER_UNIT = WAVE_HEIGHT / (VIEW_Y_MAX - VIEW_Y_MIN)
VIEW_X_MAX = physics.Z2
VIEW_X_MIN = VIEW_X_MAX - WAVE_WIDTH / PIXELS_PER_UNIT
PROFILE_LEFT, PROFILE_RIGHT = 990, 1218
STRIP_WIDTH = 22
...
# A single fixed exposure makes the projection visible. It multiplies ALL
# intensities equally, never subtracts the minimum, and never varies with
# time or B. Bright/dim ratios are retained. The adjacent curve uses the
# identical absolute intensity scale as pane three, including its zero.
DETECTOR_EXPOSURE = .92 / float(np.max(DETECTOR_INTENSITY))
DETECTOR_PIXELS = np.asarray(np.clip(DETECTOR_INTENSITY[:, None] * DETECTOR_EXPOSURE *
                                    np.asarray((255., 250., 238.))[None, :], 0, 255), dtype=np.uint8)
...
def wave_frame(field, index):
    ...
    draw.text((42, 27), "49 slits · one wave field", font=TITLE_FONT, fill=SCREEN_INK)
    draw.text((42, 68), "Waves combine at every point on the detector", font=SMALL_FONT, fill=desktop.old.MUTED)
    wall_x, _ = xy(0, 0)
    ...
    draw.text((wall_x-27, 86), "49 slits", font=CAPTION_FONT, fill=SCREEN_INK)
    detector_x, _ = xy(physics.Z2, 0)
    # Projection-screen brightness is the arriving |sum|^2, not the red/blue
    # instantaneous field. Use a wide enough strip to see the fringe envelope.
    strip = np.repeat(DETECTOR_PIXELS[:, None, :], STRIP_WIDTH, axis=1)
    image.paste(Image.fromarray(strip), (round(detector_x), WAVE_TOP))
    draw.rectangle((detector_x, WAVE_TOP, detector_x+STRIP_WIDTH, WAVE_BOTTOM),
                   outline=desktop.old.MUTED + (180,), width=1)
    draw.text((detector_x-23, 86), "detector", font=CAPTION_FONT, fill=SCREEN_INK)
    draw.text((PROFILE_LEFT+32, 86), "intensity  |sum|²", font=CAPTION_FONT, fill=SCREEN_INK)
    x_at = lambda value: PROFILE_LEFT + (PROFILE_RIGHT-PROFILE_LEFT) * float(value) / desktop.IMAX
    y_at = lambda value: xy(physics.Z2, float(value))[1]
    top, bottom = y_at(3.2), y_at(-3.2)
    draw.line((PROFILE_LEFT, top, PROFILE_LEFT, bottom), fill=desktop.old.MUTED+(100,), width=1)
    # Same data AND zero-based linear horizontal scale as the schematic pane.
    points = [(x_at(i), y_at(y)) for y, i in zip(desktop.B_VALUES, desktop.INTENSITIES)]
    draw.polygon([(PROFILE_LEFT, top), *points, (PROFILE_LEFT, bottom)], fill=desktop.old.BLUE+(18,))
    draw.line(points, fill=desktop.old.BLUE, width=2)
    for j in range(0, len(desktop.B_VALUES)-3, 7):
        draw.line((x_at(desktop.FREE_INTENSITIES[j]), y_at(desktop.B_VALUES[j]),
                   x_at(desktop.FREE_INTENSITIES[j+3]), y_at(desktop.B_VALUES[j+3])),
                  fill=desktop.old.MUTED+(105,), width=1)
    b = 0.0
    marker = (x_at(abs(physics.total(b))**2), y_at(b))
    draw.line((detector_x+STRIP_WIDTH+4, marker[1], marker[0], marker[1]), fill=desktop.old.GOLD+(210,), width=2)
    draw.ellipse((marker[0]-4, marker[1]-4, marker[0]+4, marker[1]+4), fill=desktop.old.GREEN)
    ...
    draw.text((42, 675), "Moving colors: wave phase     ·     Screen brightness and blue curve: intensity", font=CAPTION_FONT, fill=desktop.old.MUTED)
    draw.text((1094, 675), "gray: no screen", font=CAPTION_FONT, fill=desktop.old.MUTED)
    return image
...
def model_checks(field):
    ...
    return {
        ...
        "detector_strip_vs_schematic_profile_max_error": profile_error,
        "detector_brightness": "one fixed multiplicative linear exposure; no minimum subtraction or per-frame normalization",
